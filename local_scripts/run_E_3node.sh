#!/bin/bash
# Condition E: each block in isolation, no compact_kv between blocks
# 3-node (12 GPU) DDP, W=512 P=64 K=8
...
echo "Starting E (isolation) 3-node DDP: master=$MASTER node_rank=$NODE_RANK at $(date) on $(hostname)"
torchrun \
  --nnodes=3 \
  --nproc_per_node=4 \
  --node_rank=$NODE_RANK \
  ...
  --output_dir outputs/ddp_scaleup_W512_E 2>&1
echo "Finished E node_rank=$NODE_RANK at $(date)"
