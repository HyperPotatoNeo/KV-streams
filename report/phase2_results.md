...
**BPTT depth is critical.** K=8 (28%) dramatically outperforms K=1 (8%). With K=1, the compaction embeddings only receive gradient signal from one block ahead, which is insufficient to learn what information future blocks need. K=8 provides 8 blocks of lookahead, giving the compaction mechanism enough signal to learn effective compression.
...
**B(K=1) at 8% is the worst performer.** Despite being trained with compaction, the shallow BPTT gives the compaction embeddings too little learning signal. The result is barely above chance and worse than the training-free attention matching approach when you consider that B(K=1) also has the RoPE-consistent compaction advantage.
...
2. **Deep BPTT is essential.** K=8 >> K=1 (28% vs 8%). The compaction mechanism needs multi-block gradient flow to learn what information to preserve.
